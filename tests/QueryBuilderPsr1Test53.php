...
class QueryBuilderPsr1Test53 extends PHPUnit_Framework_TestCase
{
  ...
  public function testRawHaving()
  {
    ORM::forTable('widget')
       ->groupBy('name')
       ->havingRaw('`name` = ? AND (`age` = ? OR `age` = ?)', array('Fred', 5, 10))
       ->findMany();
  ...
  public function testRawWhereClauseInMethodChain()
  {
    ORM::forTable('widget')
       ->where('age', 18)
       ->whereRaw('(`name` = ? OR `name` = ?)', array('Fred', 'Bob'))
       ->where('size', 'large')
       ->findMany();
  ...
  {
    ORM::forTable('widget')
       ->rawQuery('SELECT `w`.* FROM `widget` w WHERE `name` = ? AND `age` = ?', array('Fred', 5))
       ->findMany();
  ...
  {
    ORM::forTable('widget')
       ->leftOuterJoin('widget_handle', array('widget_handle.widget_id', '=', 'widget.id'))
       ->findMany();
  ...
  {
    ORM::forTable('widget')
       ->rightOuterJoin('widget_handle', array('widget_handle.widget_id', '=', 'widget.id'))
       ->findMany();
  ...
  {
    ORM::forTable('widget')
       ->fullOuterJoin('widget_handle', array('widget_handle.widget_id', '=', 'widget.id'))
       ->findMany();
