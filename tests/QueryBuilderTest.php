...
class QueryBuilderTest extends PHPUnit_Framework_TestCase
{
  ...
  public function testOrderByWithSqlInjection()
  {
    ORM::for_table('widget')->order_by_desc('name\\`\';UPDATE USER SET TYPE="admin" WHERE ID=23;')->find_one();
    $expected = 'SELECT * FROM `widget` ORDER BY `name\``\'\';UPDATE USER SET TYPE="admin" WHERE ID=23;` DESC LIMIT 1';
    self::assertEquals($expected, ORM::get_last_query());
  }
  ...
  public function test_quote_multiple_identifiers_part()
  {
    ...
    self::assertEquals($expected, $record->_quote_identifier($record->_get_id_column_name()));
  }
